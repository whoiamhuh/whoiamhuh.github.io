    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex m-2 p-2">
                <a href="{{ route ('admin.tables.index') }}" class="px-4 py-2 bg-indigo-500 hover:bg-indigo-700 rounded-lg text-white">Создать стол</a>
            </div>
               ...
               <div class="sm:col-span-6 pt-5">
                  ...
                  <label for="address" class="block text-sm font-medium text-gray-700">Адрес</label>
                   <div class="mt-1">
                   <input type="text" id="address" name="address" value="{{ $table->address }}"
                          class="block w-full transition duration-150 ease-in-out appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
                   </div>
                   @error('address')
                        <div class="text-sm text-red-400">{{$message}}</div>
                   ...
                   <div class="sm:col-span-6 pt-5">
                  <label for="description" class="block text-sm font-medium text-gray-700">Рестораны</label>
                   <div class="mt-1">
                   <select id="restaurants" name="restaurants[]" class="form-multiselect block w-full mt-1" 
                   multiple>
                    @foreach ($restaurants as $restaurant)
                        <option value="{{ $restaurant->id }}"  @selected($table->restaurants->contains($restaurant))>{{ $restaurant->name }}</option>
